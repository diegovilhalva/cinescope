...
const Shows = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchShows(page, sort, sort === "vote_average.desc" ? 1000 : null)
      .then((res) => {
  ...
  return (
    <Container maxW={'container.xl'}>
      <Flex alignItems={'baseline'} gap={'4'} my={10}>
        <Heading as={'h2'} fontSize={'md'} textTransform={'uppercase'} mb="1">Descubra novas Séries</Heading>
        <Select w={'150px'} value={sortBy} onChange={handleSortChange}>
          <option value="popularity.desc">Populares</option>
          <option value="vote_average.desc">Top Avaliadas</option>
        </Select>
      </Flex>
      <Grid
        templateColumns={{
